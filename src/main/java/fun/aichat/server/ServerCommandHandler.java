...
public class ServerCommandHandler {
        ...
        dispatcher.register(CommandManager.literal("ai")
                ...
                .then(CommandManager.literal("list")
                        .executes(context -> {
                            COMMAND_EXECUTOR.submit(() -> {
                                ...
                                List<String> models = fun.aichat.AiModelManager.getCachedModels();
                                if (!models.isEmpty()) {
                                    for (String model : models) {
                                        final String modelName = model;
                                        context.getSource().sendFeedback(() -> Text.literal("  - " + modelName), false);
                                    }
                                    context.getSource().sendFeedback(() -> Text.translatable("command.ai.status.list_success"), false);
                                } else {
                                    // Ollama 模式下回退到命令行
                ...
                .then(CommandManager.literal("serve")
                        .executes(context -> {
                            ...
                            context.getSource().sendFeedback(() -> Text.translatable("command.ai.status.serve_starting"), false);
                            return 1;
                        }))
                // ps - 列出运行中的模型
                .then(CommandManager.literal("ps")
                        .executes(context -> {
                            ...
                            context.getSource().sendFeedback(() -> Text.translatable("command.ai.status.ps_running"), false);
                            return 1;
                        }))
                // refresh - 刷新模型列表
                .then(CommandManager.literal("refresh")
                        .executes(context -> {
                            COMMAND_EXECUTOR.submit(() -> {
                                fun.aichat.AiModelManager.refreshModels();
                                List<String> models = fun.aichat.AiModelManager.getCachedModels();
                                context.getSource().sendFeedback(() -> Text.translatable("command.ai.status.models_refreshed_api", models.size()), false);
                            });
                            return 1;
                        }))
                // model - 设置模型
                        ...
                        .then(CommandManager.argument("modelname", StringArgumentType.greedyString())
                                ...
                                .executes(context -> {
                                    ...
                                    if (fun.aichat.AiModelManager.isModelValid(modelName)) {
                                        ...
                                        context.getSource().sendFeedback(() -> Text.translatable("command.ai.status.model_set", modelName), false);
                                    } else {
                ...
                .then(CommandManager.literal("api")
                        .executes(context -> {
                            ...
                            context.getSource().sendFeedback(() -> Text.translatable("command.ai.status.api_status", currentUrl), false);
                            return 1;
                        })
                        .then(CommandManager.argument("url", StringArgumentType.greedyString())
                                .executes(context -> {
                                    ...
                                    context.getSource().sendFeedback(() -> Text.translatable("command.ai.status.api_set_with_provider", url, provider), false);
                                    return 1;
                                })))
                ...
                .then(CommandManager.literal("provider")
                        .executes(context -> {
                            ...
                            context.getSource().sendFeedback(() -> Text.translatable("command.ai.status.provider_status", current.name()), false);
                            return 1;
                        })
                        .then(CommandManager.literal("ollama")
                                .executes(context -> {
                                    fun.aichat.AiConfig.setApiProvider(fun.aichat.AiConfig.ApiProvider.OLLAMA);
                                    context.getSource().sendFeedback(() -> Text.translatable("command.ai.status.provider_set", "Ollama"), false);
                                    return 1;
                                }))
                        .then(CommandManager.literal("openai")
                                .executes(context -> {
                                    fun.aichat.AiConfig.setApiProvider(fun.aichat.AiConfig.ApiProvider.OPENAI);
                                    context.getSource().sendFeedback(() -> Text.translatable("command.ai.status.provider_set", "OpenAI Compatible"), false);
                                    return 1;
                                }))
                        .then(CommandManager.literal("lmstudio")
                                .executes(context -> {
                                    fun.aichat.AiConfig.setApiProvider(fun.aichat.AiConfig.ApiProvider.LMSTUDIO);
                                    context.getSource().sendFeedback(() -> Text.translatable("command.ai.status.provider_set", "LM Studio"), false);
                                    return 1;
                                })))
                // key - 设置 API Key
                .then(CommandManager.literal("key")
                        .executes(context -> {
                            ...
                            context.getSource().sendFeedback(() -> Text.translatable("command.ai.status.key_status", masked), false);
                            return 1;
                        })
                        .then(CommandManager.argument("apikey", StringArgumentType.greedyString())
                                .executes(context -> {
                                    ...
                                    context.getSource().sendFeedback(() -> Text.translatable("command.ai.status.key_set"), false);
                                    return 1;
                                })))
                // think - 深度思考开关
                .then(CommandManager.literal("think")
                        .executes(context -> {
                            ...
                            context.getSource().sendFeedback(() -> Text.translatable("command.ai.status.think_status", enabled ? "ON" : "OFF"), false);
                            return 1;
                        })
                        .then(CommandManager.literal("on")
                                .executes(context -> {
                                    fun.aichat.AiModelManager.setThinkEnabled(true);
                                    context.getSource().sendFeedback(() -> Text.translatable("command.ai.status.think_enabled"), false);
                                    return 1;
                                }))
                        .then(CommandManager.literal("off")
                                .executes(context -> {
                                    fun.aichat.AiModelManager.setThinkEnabled(false);
                                    context.getSource().sendFeedback(() -> Text.translatable("command.ai.status.think_disabled"), false);
                                    return 1;
                                })))
                // search - 在线搜索开关
                .then(CommandManager.literal("search")
                        .executes(context -> {
                            ...
                            context.getSource().sendFeedback(() -> Text.translatable("command.ai.status.search_status", enabled ? "ON" : "OFF"), false);
                            return 1;
                        })
                        .then(CommandManager.literal("on")
                                .executes(context -> {
                                    fun.aichat.AiModelManager.setSearchEnabled(true);
                                    context.getSource().sendFeedback(() -> Text.translatable("command.ai.status.search_enabled"), false);
                                    return 1;
                                }))
                        .then(CommandManager.literal("off")
                                .executes(context -> {
                                    fun.aichat.AiModelManager.setSearchEnabled(false);
                                    context.getSource().sendFeedback(() -> Text.translatable("command.ai.status.search_disabled"), false);
                                    return 1;
                                })))
                // context - 设置上下文轮数
                .then(CommandManager.literal("context")
                        .executes(context -> {
                            ...
                            context.getSource().sendFeedback(() -> Text.translatable("command.ai.status.context_status", rounds), false);
                            return 1;
                        })
                        .then(CommandManager.argument("rounds", IntegerArgumentType.integer(0, 50))
                                .executes(context -> {
                                    ...
                                    context.getSource().sendFeedback(() -> Text.translatable("command.ai.status.context_set", rounds), false);
                                    return 1;
                                })))
                // history - 聊天历史管理
                .then(CommandManager.literal("history")
                        .executes(context -> {
                            ...
                            context.getSource().sendFeedback(() -> Text.translatable("command.ai.status.history_count", count), false);
                            return 1;
                        })
                        .then(CommandManager.literal("clear")
                                .executes(context -> {
                                    ...
                                    if (success) {
                                        context.getSource().sendFeedback(() -> Text.translatable("command.ai.status.history_cleared"), false);
                                    } else {
    ...
    private static void showHelp(ServerCommandSource source) {
        for (int i = 1; i <= 12; i++) {
            final int lineNum = i;
            source.sendFeedback(() -> Text.translatable("command.ai.help.line" + lineNum), false);
        }
    }
        ...
        try {
            ...
            Thread errorThread = new Thread(() -> {
                try (BufferedReader errorReader = new BufferedReader(new InputStreamReader(process.getErrorStream()))) {
                    ...
                    source.sendFeedback(() -> Text.translatable("command.ai.error.generic"), false);
                } finally {
            ...
            if (!process.waitFor(60, TimeUnit.SECONDS)) {
                source.sendFeedback(() -> Text.translatable("command.ai.error.timeout"), false);
                process.destroy();
